...
def _add_pipeline_subparser(subparsers) -> None:
    ...
    p.add_argument(
        "--blastn-viral-db-manifest",
        type=Path,
        default=None,
        metavar="PATH",
        help="Optional JSON manifest for --blastn-viral-db (from build-viral-db). Validates DB checksums before BLAST.",
    )
    p.add_argument(
        "--blastn-require-viral-db-sha256",
        type=str,
        default=None,
        metavar="HEX",
        help="Optional required aggregate SHA256 fingerprint for --blastn-viral-db. Fails on mismatch.",
    )
    p.add_argument(
        "--filter-similar",
        action="store_true",
...
def _add_blastn_filter_subparser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Optional JSON manifest generated by build-viral-db; validates DB file checksums before BLAST.",
    )
    p.add_argument(
        "--require-viral-db-sha256",
        type=str,
        default=None,
        metavar="HEX",
        help="Optional required aggregate SHA256 fingerprint of the viral DB (from manifest). Fails on mismatch.",
    )
...
def _add_temporal_pipeline_subparser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Optional JSON manifest for --viral-db (from build-viral-db). Validates DB checksums before BLAST.",
    )
    p.add_argument(
        "--require-viral-db-sha256",
        type=str,
        default=None,
        metavar="HEX",
        help="Optional required aggregate SHA256 fingerprint for --viral-db. Fails on mismatch.",
    )
...
def _run_blastn_filter(args) -> None:
    ...
    if getattr(args, "viral_db_manifest", None) is not None and viral_db is None:
        raise SystemExit("--viral-db-manifest requires --viral-db")
    if getattr(args, "require_viral_db_sha256", None) is not None and viral_db is None:
        raise SystemExit("--require-viral-db-sha256 requires --viral-db")
    ...
    if getattr(args, "viral_db_manifest", None) is not None and not args.viral_db_manifest.exists():
        raise SystemExit(f"--viral-db-manifest not found: {args.viral_db_manifest}")
...
def _run_pipeline(args) -> None:
    ...
    if getattr(args, "run_blastn_filter", False):
        ...
        bv_manifest = getattr(args, "blastn_viral_db_manifest", None)
        bv_sha = getattr(args, "blastn_require_viral_db_sha256", None)
        ...
        if bv_manifest is not None and bv_db is None:
            raise SystemExit("--blastn-viral-db-manifest requires --blastn-viral-db")
        if bv_sha is not None and bv_db is None:
            raise SystemExit("--blastn-require-viral-db-sha256 requires --blastn-viral-db")
        if bv_manifest is not None and not bv_manifest.exists():
            raise SystemExit(f"--blastn-viral-db-manifest not found: {bv_manifest}")
        eve_intervals = run_blastn_from_dirs(
            download_dir,
            blastn_out,
...
def _run_temporal_pipeline(args) -> None:
    ...
    if getattr(args, "viral_db", None) is not None and args.viral_db:
        ...
        viral_db_manifest = getattr(args, "viral_db_manifest", None)
        required_sha = getattr(args, "require_viral_db_sha256", None)
        ...
        if viral_db_manifest is not None and not viral_db_manifest.exists():
            raise SystemExit(f"--viral-db-manifest not found: {viral_db_manifest}")

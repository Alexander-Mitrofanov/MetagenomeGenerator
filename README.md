...
CHIMERA generates synthetic metagenome datasets (FASTA/FASTQ) for training and evaluating sequence classifiers, for example viral versus prokaryotic or phage versus bacterial. You can use genomes from NCBI RefSeq or your own FASTA files in category folders (bacteria, virus, archaea, plasmid). The pipeline splits genomes into fixed- or variable-length reads or contigs and can apply sequencing-error and mutation models. Output is one metagenome file with traceable read IDs and optional ground-truth abundance. Reproducibility: date-stamped accession snapshots and fixed seeds let you rerun the same genome set anytime. Train/test: temporal or random splits with similarity-based filtering reduce leakage from near-duplicate sequences. EVE handling: endogenous viral elements in non-viral genomes can be excluded or exported so they do not bias classifiers. Benchmarks: fixed genome counts per category and optional replicates give comparable, standardized datasets. In-house data: place your FASTAs in the right folders and run without any NCBI download. For workflows and commands, see [Use cases at a glance](#use-cases-at-a-glance).

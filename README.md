...
CHIMERA is a pipeline for building synthetic metagenome datasets (FASTA/FASTQ) to train and evaluate sequence classifiers, such as viral versus prokaryotic or phage versus bacterial. It takes reference genomes from NCBI RefSeq or your own FASTA files in category folders (bacteria, virus, archaea, plasmid), splits them into fixed- or variable-length reads or contigs, and can apply sequencing-error and mutation models. Output is a single metagenome file with traceable read IDs and optional ground-truth abundance.

CHIMERA is built to support reproducible research. Date-stamped accession snapshots and fixed random seeds ensure that the same genome set and the same read sampling can be rerun at any time, so results can be replicated across machines and over time. For train/test evaluation, contamination in the test set is a known pitfall: if train and test share near-identical sequences (e.g. different strains of the same species), metrics look better than they should. CHIMERA supports temporal splits (train on older accessions, test on newer ones) or random splits with similarity-based filtering, so that test reads highly similar to training data can be removed and evaluation reflects true generalization. Endogenous viral elements (EVEs) in bacterial or archaeal genomes can mislead viral classifiers; the tool can exclude or export those regions so non-viral genomes stay clean. For method comparison, fixed genome counts per category and optional replicates yield standardized, comparable benchmark datasets. If you work with in-house isolates or assemblies, you can drop your FASTAs into the right folders and run the pipeline without any NCBI download. For workflows and commands, see [Use cases at a glance](#use-cases-at-a-glance).
